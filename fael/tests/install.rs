...
fn install_all_three_idempotent_and_replaces_fapony_on_request() {
    ...
    std::fs::write(&codex_cfg, "model = \"x\"\n\n[mcp_servers.fapony]\ncommand = \"bun\"\n").unwrap();
    std::fs::write(&oc, "{\n  // mine\n  \"mcp\": {\n    \"fapony\": {\"type\": \"local\"}\n  }\n}\n").unwrap();
    std::fs::write(home.join(".config/opencode/plugins/fapony-session-start.ts"), "").unwrap();

    // dry run writes nothing
    let out = install(&home, &["--dry-run"]);
    assert!(out.contains("would write") && out.contains("--replace-fapony"), "{out}");
    ...
    for sub in ["stop", "session-start", "read", "edit"] {
        assert!(s.contains(&format!(" hook {sub} --client claude")), "{sub}: {s}");
    }
    assert!(s.contains("Edit|Write|MultiEdit|NotebookEdit") && s.contains("hook-stop"), "{s}");
    let h = read(&codex_hooks);
    assert!(h.contains("hook edit --client codex") && !h.contains("hook read --client codex"), "{h}");
    let t = read(&codex_cfg);
    assert!(t.contains("[mcp_servers.fael]\ncommand = \"/") && t.contains("[mcp_servers.fapony]"), "{t}");
    let o = read(&oc);
    assert!(o.contains("// mine") && o.contains("\"fael\": {\"type\": \"local\""), "{o}");
    let p = read(&home.join(".config/opencode/plugins/fael.js"));
    assert!(p.contains("const FAEL = \"/") && !p.contains("__FAEL__"), "{p}");
    ...
    let before = [read(&claude), read(&codex_hooks), read(&codex_cfg), read(&oc)];
    let out = install(&home, &[]);
    assert!(!out.contains("wrote"), "{out}");
    assert_eq!(before, [read(&claude), read(&codex_hooks), read(&codex_cfg), read(&oc)]);

    // --replace-fapony: blockers + MCP out, everything else of fapony stays
    install(&home, &["--replace-fapony"]);
    let s = read(&claude);
    assert!(!s.contains("hook-stop") && s.contains("hook-read-hint") && s.contains("hook stop --client claude"), "{s}");
    assert!(!read(&codex_hooks).contains("hook-stop"));
    let t = read(&codex_cfg);
    assert!(!t.contains("fapony") && t.contains("[mcp_servers.fael]"), "{t}");
    assert!(read(&oc).contains("\"fapony\": {\"enabled\": false,"));
    assert!(home.join(".config/opencode/plugins/fapony-session-start.ts.disabled").is_file());
}
